"""Environment bootstrap utilities for skill route agent."""
...
def bootstrap_env() -> None:
    """Load skill-route .env first, then repo root .env as fallback."""
    agent_dir = Path(__file__).resolve().parent
    repo_root = agent_dir.parents[1]

    # Prefer agent-local environment values when present.
